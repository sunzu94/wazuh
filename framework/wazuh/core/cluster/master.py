...
from task_pool_functions import *
...
class MasterHandler(server.AbstractServerHandler, c_common.WazuhCommon):
    ...
    async def sync_wazuh_db_info(self, task_id: bytes):
        ...
        try:
            task = self.loop.run_in_executor(
                ...
                partial(send_data_to_wdb, data, self.cluster_items['intervals']['master']['timeout_agent_info'])
            )
    ...
    async def sync_worker_files(self, task_id: str, received_file: asyncio.Event, logger):
        ...
        try:
            task = self.loop.run_in_executor(
                self.server.task_pool,
                partial(process_files_from_worker, files_metadata, decompressed_files_path, self.cluster_items,
